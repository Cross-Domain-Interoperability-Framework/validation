...
| `converters/DCAT/dcat_to_cdif.py` | DCAT JSON-LD → CDIF. **Table-driven**: reads `converters/mappings/dcat-to-cdif.sssom.tsv` (174 rows, every DCAT property plus every one in the corpus) and `dcat-aliases.sssom.tsv` at import, so the table and the behaviour cannot drift. Two extension columns do the work — `subject_class` (DCAT is a graph: `dcterms:title` differs on a Dataset and a Distribution) and `transform` (the shaper; empty = plain copy). Resolves `@id` node references against the whole document before mapping — real DCAT barely nests. `conformsTo` derived from content via detect_conformance (`--static-conformance` opts out). See `converters/DCAT/README.md` |
