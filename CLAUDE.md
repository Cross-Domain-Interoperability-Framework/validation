...
| `converters/DDI/ddi_sssom_to_cdif.py` | Data-driven DDI → CDIF converter: applies every mapping in the SSSOM worksheets (`converters/mappings`, via `ddi_mappings.json`) and delegates the value-domain/code-list/statistics + contributor/catalog-record/rights construction to `ddi122_to_cdif.py`. `--version 122` (1.2.2) or `25` (2.5). **Generates the committed example outputs** under `Examples/cdif/` |
