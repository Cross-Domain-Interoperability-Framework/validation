...
- **SHACL**: **cdifbook 10/10 clean.** The `testJSONMetadata` (77) and `cdifProfiles` (5) groups carry violations from the **IRI-lint** shapes added by the automated shape regeneration (`AdditionalTypeUriShouldBeIRIShape` / `PropertyIDUriShouldBeIRIShape`): a `schema:additionalType`/`schema:propertyID` value shaped like a URI/CURIE (`^[A-Za-z][A-Za-z0-9+.\-]*:[^\s"]+$`) must be an `{"@id":…}` IRI, not a string (free labels like `MaterialSample` stay valid).
  - **`additionalType` — cleaned.** The `testJSONMetadata` corpus was normalized (the NeXus `nxs:` type CURIEs → `{"@id":…}`), dropping each file from 16 → 12 batch violations (batch sums per declared profile). The DDI-converter examples and `cdifbook` were already clean.
  - **`propertyID` — blocked by a shapes conflict (residual).** On an identifier `schema:PropertyValue`, `resourceIdentifierProperty` requires `schema:propertyID` to be `xsd:string`, while `PropertyIDUriShouldBeIRIShape` wants it as an `{"@id":…}` IRI — a URI-valued identifier scheme cannot satisfy both, so the corpus is left as-is and the fix belongs in the building-block shapes (Layer A), not the data. This is the residual `testJSONMetadata`/`cdifProfiles` violation.
- Non-lint SHACL findings are advisory warnings/info (missing activity descriptions, contact points, physical data types, etc.).
