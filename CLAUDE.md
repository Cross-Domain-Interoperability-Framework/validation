...
| `converters/DCAT/dcat_to_cdif.py` | DCAT JSON-LD → CDIF. **Table-driven**: reads `converters/mappings/dcat-to-cdif.sssom.tsv` (256 rows, every DCAT property plus every one in the corpus) and `dcat-aliases.sssom.tsv` at import, so the table and the behaviour cannot drift. Two extension columns do the work — `subject_class` (DCAT is a graph: `dcterms:title` differs on a Dataset and a Distribution) and `transform` (the shaper; empty = plain copy). Resolves `@id` node references against the whole document before mapping — real DCAT barely nests. `schema:identifier` falls back to the dataset `@id` when the source has no `dcterms:`/`adms:` identifier (so every dataset meets core); a post-build pass declares every CURIE prefix the record uses so it frames. `conformsTo` derived from content via detect_conformance (`--static-conformance` opts out). All 239 corpus records in `cdifOK/` are core-conformant and validate. See `converters/DCAT/README.md` |
| `converters/DCAT/build_corpus.py` | Rebuilds `cdifOK/` from `dcatExamplesOK/`: groups by directory+stem, merges every serialization into one graph, converts once. Verifies on each run that **no source predicate is dropped** and that **every conformant record validates**; both report zero. `--check` verifies without writing. |
| `converters/DDI/ddi2cdif.py` | **Single DDI→CDIF entry point.** Sniffs the input's flavor (root element / `version` attr / namespace, or JSON-LD) and dispatches: **DDI Codebook 1.2.2 & 2.5** → the data-driven engine `ddi_sssom_to_cdif.py` (version auto-chosen); **DDI-CDI** (RDF/JSON-LD) and **DDI Lifecycle 3.x** (`<DDIInstance>`) → explicit separate branches that raise a clear not-implemented error. `--print-flavor` reports the detection. Also `sniff_source()`/`--print-source` → `dataverse`\|`nada`\|`generic` (a producer axis below flavor, using producer branding not overlapping structural conventions). The three peers: **Codebook (common + version wrappers) \| DDI-CDI \| (future) Lifecycle 3.x**. The engine reads a resolvable per-file download URL from `<fileDscr URI>` (Dataverse) for `schema:contentUrl`, else the OGC nil. Dataverse fixture: `DDICodebook/Examples/{XML,cdif}/…GlobalPartySurvey-2019…` |
| `converters/DDI/ddi_to_cdif.py` | **Source-specific** Harvard Dataverse 2.5 variant, deliberately outside `ddi2cdif.py`: adds live Dataverse-API enrichment (file size/checksum, HTTP headers via `--fetch-file-meta`/`--fetch-headers`) that the offline engine lacks. Its base conversion is superseded by `ddi_sssom_to_cdif.py`; a future refactor could make it an enrichment pass over the engine's output |
| `converters/DDI/ddi122_to_cdif.py` | Hand-coded **DDI Codebook 1.2.2** (ICPSR/Nesstar) → CDIF converter (OHDSI/Malawi). Builds the full study/variable/file structure incl. enumerated value domains + shared deduplicated code lists + statistics, structured `schema:contributor` (Role/Organization/ContactPoint), a typed `dcat:CatalogRecord`, nil `contentUrl`/`license` placeholders, and a `schema:dateModified` fallback. Profile scope via detect_conformance. See `converters/DDI/README.md` |
| `converters/DDI/ddi_sssom_to_cdif.py` | Data-driven DDI → CDIF converter: applies every field mapping in the SSSOM worksheets (`converters/mappings`, via `ddi_mappings.json`; bare-node type-declaration rows like `dataDscr.var`→`$.schema:variableMeasured[*]` are skipped — they have no scalar property to set) and delegates the value-domain/code-list/statistics + contributor/catalog-record/rights construction to `ddi122_to_cdif.py`. `--version 122` (1.2.2) or `25` (2.5). **Generates the committed example outputs** under `Examples/cdif/` |
| `converters/DDICodebook/ddi25_to_cdif.py` | Thin **DDI Codebook 2.5** shim over `ddi_sssom_to_cdif.py` (`--version 25`), keeping a 2.5-named entry point; byte-identical to running the engine directly. 2.5 is an element-level superset of 1.2.2, so there is one DDI→CDIF engine. See `converters/DDICodebook/README.md` |
...
- `MetadataExamples/` - Sample CDIF metadata files
  - `cdifComplete-example.json` -- Copy of the `cdifComplete` building-block composite example; declares a `cdif:` prefix in its `@context` and full conformance set (core/discovery/data_description/manifest/provenance/data_structure). Regression fixture for the `FrameAndValidate` fix that re-expands compacted `cdif:` `@id` values (e.g. `cdif:data_description/1.1` → `https://w3id.org/cdif/data_description/1.1`) so framed conformsTo matches the schema's expanded-URI `const`. Frames + validates against `CDIFCompleteSchema.json`.
...
- **SHACL**: **89/92 clean** — `testJSONMetadata` 77/77 and `cdifbook` 10/10 are clean; only 3 `cdifProfiles` building-block examples (`exampleCDIFDiscovery`, `exampleCDIFDataDescription`, `exampleCDIFDataStructureComplete`) carry non-lint violations (sibling-repo examples, out of the OHDSI scope). Getting there took three coordinated fixes for the **IRI-lint** shapes (`AdditionalTypeUriShouldBeIRIShape` / `PropertyIDUriShouldBeIRIShape` — a `schema:additionalType`/`schema:propertyID` value shaped like a URI/CURIE, `^[A-Za-z][A-Za-z0-9+.\-]*:[^\s"]+$`, must be an `{"@id":…}` IRI, not a string; free labels like `MaterialSample` stay valid):
  - **corpus:** `testJSONMetadata` `schema:additionalType` (NeXus `nxs:` CURIEs) and `schema:propertyID` (identifier scheme URIs) normalized to `{"@id":…}`.
  - **building-block shapes:** the `propertyID` IRI form conflicted with `resourceIdentifierProperty`, which required the identifier's `schema:propertyID` to be `xsd:string`. Fixed in `metadataBuildingBlocks` (cdifCore + xasDocument `rules.shacl`) to accept `xsd:string` **or** an IRI.
  - **shape regeneration:** the four intermediate profile shapes (`CDIF-DataDescription`/`DataStructure`/`Provenance`/`Manifest-Shapes.ttl`) had drifted stale (2026-06) because the `update-validation-schemas` CI workflow regenerated only `CDIF-Discovery`/`CDIF-Complete-Shapes.ttl`; they were regenerated by hand to propagate the lint + fix, and **the CI workflow (metadataBuildingBlocks `.github/workflows/update-validation-schemas.yml`) was updated to regenerate all six profile shapes**, so they no longer drift.
...
`apply_conformance(doc, uris)` rewrites `schema:subjectOf/dcterms:conformsTo` to the detected `cdif:` URIs **while preserving any claim outside the `https://w3id.org/cdif/` namespace** (e.g. `ada:profile/...` domain profiles). This is wired into every `format → CDIF` converter — `converters/croissant/ConvertFromCroissant.py`, `converters/soso/ConvertFromSOSO.py` (and thus `converters/soso2cdif.py`), `converters/DCAT/dcat_to_cdif.py`, and `converters/DDI/ddi_to_cdif.py` (each with a `--static-conformance` opt-out) — and into `amds-ldeo`'s `ada_json_loader.py` (post-build hook, preserving each record's domain profile).
