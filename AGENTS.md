...
- **Croissant Conversion (bidirectional)** -- `croissant/ConvertToCroissant.py` converts CDIF → Croissant 1.0 (`mlcommons.org/croissant/1.0`); `croissant/ConvertFromCroissant.py` converts Croissant → CDIF DataDescription/Discovery (lossy inverse: reconstructs `schema:identifier` from DOI in `citeAs`/`url`, pivots `cr:FileObject`+`containedIn` archive idiom into `schema:DataDownload`+`hasPart`, generates `schema:variableMeasured` (`cdi:InstanceVariable`) + per-file `cdi:hasPhysicalMapping` from `cr:RecordSet`/`cr:Field`, emits a `schema:subjectOf` stub with both Discovery and DataDescription conformance URIs, merges source `@context` prefixes so pass-through PROV/etc. blocks survive framing). When the source Croissant has no `recordSet`, the output validates against `CDIFDiscoverySchema.json` instead of `CDIFDataDescriptionSchema.json` — that is the appropriate profile in that case. Per-direction property mappings: `croissant/CDIFtoCroissant.md` (forward), `croissant/CroissantToCDIF.md` (inverse).
