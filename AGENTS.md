...
- **RO-Crate** (structural) -- `converters/ROCrate/`: ConvertToROCrate.py, ROCrateToCDIF.py, ValidateROCrate.py. (They lived in `profile-manifest/tools/` until 2026-09-08; an earlier note here pointed at a `packaging` repo that the 2026-05 reorg had already renamed.)
- **Batch** -- `batch_validate.py` runs both JSON Schema and SHACL validation across 92 files in three groups (testJSONMetadata=77, cdifbook=10, cdifProfiles=5; the former `adaProfiles` group was dropped in the BB restructure), with severity-aware reporting (violations vs warnings vs info). The cdifbook/cdifProfiles groups resolve from sibling GitHub repos — local clones by default, or HTTPS artifacts (cached in `_remote_cache/`) with `CDIF_BATCH_SOURCE=github`
- **Conformance detection** -- `detect_conformance.py` derives the profiles a document *actually* conforms to from its **content** (vs. trusting its declared `conformsTo`): each class has a presence SPARQL `ASK` (the elements it introduces beyond its base) gated by a content-SHACL validity check (only `sh:Violation` blocks; warnings/info advisory). `apply_conformance()` rewrites `schema:subjectOf/dcterms:conformsTo` with the detected `cdif:` URIs while preserving any non-`cdif:` (domain) claim. `--apply`/`--from-source` CLI; importable. `CDIF_CONFORMANCE_FROM_SOURCE=1` reads each class's rules from the building blocks' `conformance.json` sidecars (see `docs/CDIF-Conformance-Declaration-Convention.md`) rather than the in-code registry. Wired into `converters/croissant/ConvertFromCroissant.py` and `amds-ldeo`'s `ada_json_loader.py`
...
- **DCAT Conversion** -- `converters/DCAT/dcat_to_cdif.py` converts DCAT JSON-LD catalogs to CDIF schema.org format. It **reads** its mappings from `converters/mappings/dcat-to-cdif.sssom.tsv` rather than restating them, so the table is the authority (the generic applier is `converters/sssom_engine.py`, shared with the Croissant converters; row order is precedence for scalar targets, array targets accumulate); hand-written shapers, named by the table's `transform` column, do the structural work the table cannot express. Resolves `@id` node references against the whole document first (real DCAT rarely nests). Supports nested catalogs, qualified attributions (roles), spatial/temporal, distributions, services. Profile derived from content. Preserves unmapped properties (open-world). `converters/DCAT/build_corpus.py` rebuilds the example corpus and verifies it.
- **DDI Conversion** -- `converters/DDI/ddi2cdif.py` is the single entry point: it sniffs the DDI flavor and routes it. **DDI Codebook 1.2.2 & 2.5** go to the source-agnostic data-driven engine `converters/DDI/ddi_sssom_to_cdif.py` (applies the SSSOM crosswalk under `converters/mappings` and delegates the value-domain/statistics/code-list/contributor/physical-mapping construction to `converters/DDI/ddi122_to_cdif.py`); **DDI-CDI** (RDF/JSON-LD) and **DDI Lifecycle 3.x** are separate, not-yet-implemented branches. `converters/DDI/ddi_to_cdif.py` is a source-specific Harvard Dataverse tool, outside the dispatcher: it delegates the base conversion to the engine and adds only live Dataverse-API enrichment (`--fetch-file-meta`/`--fetch-headers` → file size/checksum and the `.tab` header row as `cdif:hasPhysicalMapping`). Data-structure properties use the current `cdif:` prefix (`cdif:role`, `cdif:physicalDataType`, `cdif:hasPhysicalMapping`, `cdif:index`, `cdif:formats_InstanceVariable`); the DDI-CDI node types (`cdi:InstanceVariable`, `cdi:TabularTextDataSet`) keep `cdi:`.
- **Croissant Conversion (bidirectional)** -- targets **Croissant 1.1** (`mlcommons.org/croissant/1.1`) and the current **`cdif:` schema** (`https://w3id.org/cdif/`). Both directions are **table-driven**: neither converter states a property correspondence in code -- `converters/mappings/croissant-to-cdif.sssom.tsv` and `cdif-to-croissant.sssom.tsv` are the authority, applied by the shared `converters/sssom_engine.py`, with `converters/mappings/croissant-aliases.sssom.tsv` folding wild spelling variants onto the term the table keys on. Named `transform` shapers do the structural work no table can express. The forward table also carries a target-less `transform: unmapped` row for every CDIF property Croissant has no vocabulary for, so **the loss is declared rather than discovered** and the invariant "every CDIF property either reaches the output or has a row saying it does not" is checkable. Croissant's required-field fallbacks (name/description/url/licence/datePublished/creator/version) stay in the Python -- they are the converter deciding what to do about silence, not correspondences. `converters/croissant/ConvertToCroissant.py` converts CDIF → Croissant 1.1; `converters/croissant/ConvertFromCroissant.py` converts Croissant (1.0 **or** 1.1) → CDIF DataDescription/Discovery (lossy inverse: reconstructs `schema:identifier` from DOI in `citeAs`/`url`, pivots `cr:FileObject`+`containedIn` into `schema:DataDownload`+`hasPart`, generates `schema:variableMeasured` (`cdi:InstanceVariable`) + per-file `cdif:hasPhysicalMapping` (`cdif:index`/`cdif:formats_InstanceVariable`/single-value `cdif:physicalDataType`) from `cr:RecordSet`/`cr:Field`, maps `cr:RecordSet.key` → `cdif:hasPrimaryKey`, emits the current `schema:subjectOf` catalog-record shape whose `dcterms:conformsTo` is now **content-derived** via `detect_conformance` (replacing the old hardcoded core/discovery + conditional-data_description heuristic — so e.g. Kaggle archives auto-gain `manifest/1.1` and a variable-less dataset drops `data_description`), merges source `@context` prefixes so pass-through PROV/etc. survive framing). `cdi:qualifies` is **not** mapped to `cr:Field.references` (it is metadata-about-data, not a foreign key). When the source Croissant has no `recordSet`, the output validates against the Discovery schema instead of DataDescription. Forward output gives every `cr:FileObject` a `contentUrl`/`encodingFormat`/`sha256` (placeholders when CDIF lacks them) and a RecordSet `@id` distinct from its source FileObject `@id`. Validated with `mlcroissant` (1.1) + `FrameAndValidate`. Per-direction mappings (incl. full Data Structure profile crosswalk): `converters/croissant/CDIFtoCroissant.md` (forward), `converters/croissant/CroissantToCDIF.md` (inverse). The CDIF test corpus (`MetadataExamples/`, `testJSONMetadata/`) was migrated to the current `cdif:` schema via `tools/migrate_corpus_cdi_to_cdif.py` (all 84 validate); harvested Croissant examples + conversions live in `croissant/croissantExamples/`.
...
```bash
# JSON Schema validation (frames then validates). The normative script lives in
# tools/ and is the source synced into every release repo (tools/sync_frameandvalidate.py).
python tools/FrameAndValidate.py metadata.jsonld -v --schema CDIFDiscoverySchema.json --frame CDIF-frame-2026.jsonld
...
- **Context-aware array normalization**: `FrameAndValidate.py` wraps properties in arrays based on the enclosing `@type` context, not globally. Key context-dependent properties:
  ...
  - `schema:about`: array on manifest archive/part nodes (a part pointing at the part it documents); single `{@id}` inside a `schema:subjectOf` catalog record. Keyed on `parent_key != 'schema:subjectOf'`, **not** the catalog-record marker, whose `schema:additionalType` form varies between `'dcat:CatalogRecord'` and `{'@id': 'dcat:CatalogRecord'}` across profiles
- **Multi-frame selection**: when a profile ships more than one `*-frame.jsonld`, `FrameAndValidate.py` selects the frame whose root `@type` matches the input document's root `@type` (by local name, prefix/IRI-agnostic). Single-frame profiles are unaffected (the sole frame is always used). This lets one profile validate documents with different root shapes — e.g. `cdifDataStructure` ships a Dataset-rooted frame plus a structure-rooted `*-structure-frame.jsonld` for bare `cdi:*DataStructure` documents. A structure-rooted-only pass (`normalize_bare_structure`) then collapses framing-inlined component references (`cdi:qualifies` / `cdi:refersTo`) to `{@id}` and array-wraps `cdif:name` / `cdi:qualifies` / `cdi:semantic` — gated on the document root so it never touches Dataset / manifest / data-description docs.
