...
- **Conformance detection** -- `detect_conformance.py` derives the profiles a document *actually* conforms to from its **content** (vs. trusting its declared `conformsTo`): each class has a presence SPARQL `ASK` (the elements it introduces beyond its base) gated by a content-SHACL validity check (only `sh:Violation` blocks; warnings/info advisory). `apply_conformance()` rewrites `schema:subjectOf/dcterms:conformsTo` with the detected `cdif:` URIs while preserving any non-`cdif:` (domain) claim. `--apply`/`--from-source` CLI; importable. `CDIF_CONFORMANCE_FROM_SOURCE=1` reads each class's rules from the building blocks' `conformance.json` sidecars (see `docs/CDIF-Conformance-Declaration-Convention.md`) rather than the in-code registry. Wired into `croissant/ConvertFromCroissant.py` and `amds-ldeo`'s `ada_json_loader.py`
