#!/usr/bin/env python3
"""
Generate a composite SHACL shapes file for the CDIF Discovery profile by
merging building block rules.shacl files.
...
Usage:
    python generate_shacl_shapes.py [--bb-dir PATH] [--output PATH] [-v]
...
def make_header(stats, bb_dir):
    """Return a Turtle comment block for the file header."""
    today = date.today().isoformat()
    return (
        f"# CDIF Discovery Profile -- Composite SHACL Shapes\n"
        f"# Generated by generate_shacl_shapes.py "
        ...
        f"#\n"
        f"# DO NOT EDIT -- regenerate with: python generate_shacl_shapes.py\n"
    )
...
def main():
    ...
    parser.add_argument(
        "--output", "-o",
        type=Path,
        default=Path("CDIF-Discovery-Core-Shapes.ttl"),
        help="Output Turtle file (default: CDIF-Discovery-Core-Shapes.ttl)",
    )
    parser.add_argument(
        "--verbose", "-v",
        action="store_true",
    ...
    print(f"Merging {len(CDIF_DISCOVERY_BLOCKS)} building block rule sets...\n")

    # Merge
    merged, claimed, stats = merge_shapes(
        bb_dir, CDIF_DISCOVERY_BLOCKS, verbose=args.verbose
    )
