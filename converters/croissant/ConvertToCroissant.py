...
def _walk_nodes(doc):
    """Every dict in `doc`, so a bare @id reference can be resolved to what it
    names. These documents are graphs flattened into a tree; a reference and
    its referent are often siblings several levels apart."""
    stack = [doc]
    while stack:
        node = stack.pop()
...
def _x_provtext(value, cdif, rule, cr):
    """A PROV activity as the Responsible-AI data-collection statement.

    Croissant has no provenance vocabulary, so a structured activity -- who
    ran what, when, with which instrument -- can only survive as prose. This
    keeps the prose and loses the structure, which the row records as a
    closeMatch rather than an exact one.

    Activities in the wild come two ways: with a description or name, and with
    nothing but `prov:used` node references. The second is resolved against
    the document when it names something the document also describes, and
    falls back to the reference itself rather than dropping the activity.
    """
    index = {}
    for node in _walk_nodes(cdif):
        got = node.get("@id")
        if got and (node.get("schema:name") or node.get("rdfs:label")):
            index[got] = node.get("schema:name") or node.get("rdfs:label")

    statements = []
    for act in _as_list(value):
        if isinstance(act, str):
            statements.append(index.get(act, act))
            continue
        if not isinstance(act, dict):
            continue
        prose = _get(act, "schema:description") or _get(act, "schema:name")
        if prose:
            statements.append(prose)
            continue
        used = []
        for item in _as_list(act.get("prov:used")):
            ref = item.get("@id") if isinstance(item, dict) else item
            if isinstance(item, dict):
                ref = _get(item, "schema:name") or ref
            if ref:
                used.append(index.get(ref, ref))
        if used:
            statements.append("Generated by an activity using " + ", ".join(used) + ".")
    return "\n\n".join(st for st in statements if st) or None
...
CDIF_TO_CROISSANT = _engine.MappingSet(
    ...
    transforms={
        ...
        "sameas": _x_sameas, "license": _x_license, "catalog": _x_catalog, "provtext": _x_provtext,
