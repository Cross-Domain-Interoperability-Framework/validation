...
The `generate_shacl_shapes.py` script merges building block `rules.shacl` files into a single composite Turtle file for the CDIFDiscovery profile. This is the SHACL counterpart to `generate_graph_schema.py`.

```bash
# Regenerate composite SHACL shapes from building block sources
...
The script merges 22 building block rule sets using priority ordering for conflict resolution: sub-building blocks (most specific) take precedence over composites, which take precedence over profile-level copies. Run `python generate_shacl_shapes.py -v` to see which shapes are kept from which source.
...
| File | Description |
|------|-------------|
| `CDIF-Discovery-Core-Shapes.ttl` | Composite SHACL shapes for CDIFDiscovery profile (generated from 22 building block rules) |
| `CDIF-Discovery-Core-Shapes2.ttl` | Legacy hand-maintained SHACL shapes (superseded by generated version) |
| `generate_shacl_shapes.py` | Generates the composite SHACL shapes file from building block sources |
