...
| `CDIFCompleteSchema.json` | JSON Schema for framed (tree) CDIF complete profile metadata (discovery + data description) |
...
| `CDIF-Discovery-Core-Shapes.ttl` | Composite SHACL shapes for CDIFDiscovery profile (generated by `generate_shacl_shapes.py`) |
...
```bash
python ShaclValidation/ShaclJSONLDContext.py my-metadata.jsonld CDIF-Discovery-Core-Shapes.ttl
```
...
```bash
python ShaclValidation/ShaclJSONLDContext.py -v my-metadata.jsonld CDIF-Discovery-Core-Shapes.ttl
```
...
- The framed tree schemas (`CDIFCompleteSchema.json`, `CDIFDiscoverySchema.json`) are the current defaults. The original all-in-one `CDIF-JSONLD-schema-2026.json` is in `archive/`.
