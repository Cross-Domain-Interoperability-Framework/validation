...
- [Flattened Graph Schema](#flattened-graph-schema)
  - [Building Block Sources](#building-block-sources)
  - [Graph Schema Usage](#graph-schema-usage)
  - [Schema Structure (Graph)](#schema-structure-graph)
  - [Key Transformations](#key-transformations)
- [Troubleshooting](#troubleshooting)
  - [Common Validation Errors](#common-validation-errors)
  - [Debugging](#debugging)
- [Composite SHACL Shapes](#composite-shacl-shapes)
- [SHACL Validation](#shacl-validation)
  - [SHACL Files](#shacl-files)
  - [Using ShaclJSONLDContext.py](#using-shacljsonldcontextpy)
  - [Generating SHACL Validation Reports](#generating-shacl-validation-reports)
...
| File | Description |
|------|-------------|
| `CDIFDiscoverySchema.json` | JSON Schema for framed (tree) CDIF discovery profile metadata, generated by `generate_validation_schema.py` from [CDIFDiscoveryProfile resolvedSchema](https://cross-domain-interoperability-framework.github.io/metadataBuildingBlocks/_sources/profiles/cdifProfiles/CDIFDiscoveryProfile/resolvedSchema.json) |
| `CDIFCompleteSchema.json` | JSON Schema for framed (tree) CDIF complete profile metadata (discovery + data description + archive + provenance), generated by `generate_validation_schema.py` from [CDIFcompleteProfile resolvedSchema](https://cross-domain-interoperability-framework.github.io/metadataBuildingBlocks/_sources/profiles/cdifProfiles/CDIFcompleteProfile/resolvedSchema.json) |
| `CDIFDataDescriptionSchema.json` | JSON Schema for framed (tree) CDIF data description profile metadata (discovery + data description), generated by `generate_validation_schema.py` from [CDIFDataDescriptionProfile resolvedSchema](https://cross-domain-interoperability-framework.github.io/metadataBuildingBlocks/_sources/profiles/cdifProfiles/CDIFDataDescriptionProfile/resolvedSchema.json) |
...
| `tools/FrameAndValidate.py` | Python script for framing and validation (**normative source** for the release-repo copies; see [`tools/README.md`](tools/README.md)) |
...
| `detect_conformance.py` | **Derives** which profiles a document conforms to from its *content* (rather than trusting its declared `conformsTo`): for each class, a presence SPARQL ASK (does the document use what the class introduces?) gated by a content-SHACL validity check (only `sh:Violation` blocks). `apply_conformance()` writes the detected `cdif:`-prefixed URIs back into `schema:subjectOf/dcterms:conformsTo` while preserving any non-`cdif:` (e.g. domain) profile claims. Importable; `--apply` / `--from-source` CLI. With `CDIF_CONFORMANCE_FROM_SOURCE=1` (or `--from-source`) it reads each class's rules from the building blocks' `conformance.json` sidecars instead of the in-code registry. |
...
```bash
# Using Python script (pass the schema + frame explicitly in this repo)
python tools/FrameAndValidate.py my-metadata.jsonld -v --schema CDIFDiscoverySchema.json --frame CDIF-frame-2026.jsonld
...
```bash
python tools/FrameAndValidate.py my-metadata.jsonld -o framed.json --frame CDIF-frame-2026.jsonld
```
...
File groups validated (92 files total):
- **testJSONMetadata** -- 77 ADA metadata test files
- **cdifbook** -- 10 cdifbook example documents
- **cdifProfiles** -- 5 CDIF composite-profile examples from the building blocks repo

The `cdifbook` and `cdifProfiles` groups read from sibling GitHub repos (`cdifbook`, `metadataBuildingBlocks`). Set `CDIF_BATCH_SOURCE=github` to fetch those artifacts over HTTPS (cached under `_remote_cache/`) instead of from local clones; the default `local` resolves them from `C:\GithubC\CDIF\…`. The `adaProfiles` group was dropped when the building blocks were restructured.
...
When you know exactly which profile you want, use the `FrameAndValidate.py`
shipped in that release repo. Those copies are now generated from a single
normative source — `tools/FrameAndValidate.py` here — which is profile-agnostic
(it auto-detects the one schema + frame in each repo, and its `ARRAY_PROPERTIES`
is the union across all profiles, since wrapping an absent property is a no-op).
Edit the normative source and run `python tools/sync_frameandvalidate.py --apply`
to propagate; see [`tools/README.md`](tools/README.md). When you want to ask
...
The generator reads building block schemas from the `metadataBuildingBlocks/_sources/` directory (the [BuildingBlockSubmodule](https://github.com/Cross-Domain-Interoperability-Framework/metadataBuildingBlocks)). The location is auto-detected or can be overridden:
...
In addition to JSON Schema validation, CDIF metadata can be validated using [SHACL (Shapes Constraint Language)](https://www.w3.org/TR/shacl/) rules. SHACL validation operates on the RDF graph and can express constraints that JSON Schema cannot -- SPARQL-based targeting, cross-node relationships, and semantic inference.

The composite SHACL shapes are compiled from modular `rules.shacl` files in individual building blocks in the [metadataBuildingBlocks](https://github.com/Cross-Domain-Interoperability-Framework/metadataBuildingBlocks) repository. Two profiles are available:
