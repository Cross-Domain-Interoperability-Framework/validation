...
| File | Description |
|------|-------------|
| `CDIFDiscoverySchema.json` | JSON Schema for framed (tree) CDIF discovery profile metadata, generated by `generate_validation_schema.py` from [CDIFDiscovery resolvedSchema](https://usgin.github.io/metadataBuildingBlocks/_sources/profiles/cdifProfiles/CDIFDiscovery/resolvedSchema.json) |
| `CDIFCompleteSchema.json` | JSON Schema for framed (tree) CDIF complete profile metadata (discovery + data description + provenance), generated by `generate_validation_schema.py` from [CDIFcomplete resolvedSchema](https://usgin.github.io/metadataBuildingBlocks/_sources/profiles/cdifProfiles/CDIFcomplete/resolvedSchema.json) |
...
| `CDIF-Discovery-Shapes.ttl` | Composite SHACL shapes for CDIFDiscovery profile (generated by `generate_shacl_shapes.py`) |
| `CDIF-Complete-Shapes.ttl` | Composite SHACL shapes for CDIFcomplete profile (generated by `generate_shacl_shapes.py --profile complete`) |
...
The generator reads building block schemas from the `metadataBuildingBlocks/_sources/` directory (the [BuildingBlockSubmodule](https://github.com/usgin/metadataBuildingBlocks)). The location is auto-detected or can be overridden:
...
- **discovery** (default) → `CDIF-Discovery-Shapes.ttl` — 64 shapes for the CDIFDiscovery profile
...
# With explicit paths
python generate_shacl_shapes.py --bb-dir /path/to/_sources --output CDIF-Discovery-Shapes.ttl

# Verbose output showing conflict resolution
python generate_shacl_shapes.py -v
```

The script merges building block rule sets using priority ordering for conflict resolution: sub-building blocks (most specific) take precedence over composites, which take precedence over profile-level copies. Run `python generate_shacl_shapes.py -v` to see which shapes are kept from which source.

The shapes include 3 **SOSO (Science on Schema.org) namespace-check shapes** that detect incorrect schema.org namespace variants (e.g., `https://schema.org/` instead of `http://schema.org/`, or missing trailing slash).
...
| File | Description |
|------|-------------|
| `CDIF-Discovery-Shapes.ttl` | Composite SHACL shapes for CDIFDiscovery profile (64 shapes, generated) |
| `CDIF-Complete-Shapes.ttl` | Composite SHACL shapes for CDIFcomplete profile (76 shapes, generated) |
| `CDIF-Discovery-Shapes2.ttl` | Legacy hand-maintained SHACL shapes (superseded by generated versions) |
...
```bash
python ShaclValidation/ShaclJSONLDContext.py my-metadata.jsonld CDIF-Discovery-Shapes.ttl
```
...
```bash
python ShaclValidation/ShaclJSONLDContext.py -v my-metadata.jsonld CDIF-Discovery-Shapes.ttl
```
...
### Generating SHACL Validation Reports

The `generate_shacl_report.py` script runs SHACL validation and produces a structured markdown report suitable for sharing. Issues are grouped by severity (Violation, Warning, Info) and then by constraint message, with each issue showing the focus node's type and name for context.
...
python generate_shacl_report.py metadata.jsonld CDIF-Discovery-Shapes.ttl

# Using named arguments with verbose output
...
**Report structure:**
- Header with date, file paths, triple counts, conformance status, and total issue count
- Summary table showing counts by severity level
- Detail sections for each severity, with issues grouped by constraint message
- Each issue shows the focus node described with its `@type` and `schema:name` (or `@id` for URI nodes)
